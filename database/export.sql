...
CREATE TABLE "users" ("id" integer primary key autoincrement not null, "name" varchar not null, "email" varchar not null, "email_verified_at" datetime, "password" varchar not null, "remember_token" varchar, "created_at" datetime, "updated_at" datetime, "phone" varchar, "avatar" varchar, "bio" text, "birth_date" date, "gender" varchar check ("gender" in ('male', 'female')), "city" varchar, "country" varchar not null default 'Saudi Arabia', "is_active" tinyint(1) not null default '1', "last_login_at" datetime, "google_id" varchar, "status" varchar not null default 'active');
...
CREATE TABLE "roles_and_permissions_tables" ("id" integer primary key autoincrement not null, "created_at" datetime, "updated_at" datetime);


CREATE TABLE "categories" ("id" integer primary key autoincrement not null, "name_ar" varchar not null, "name_en" varchar, "slug" varchar not null, "description_ar" text, "description_en" text, "icon" varchar, "color" varchar not null default '#10B981', "image" varchar, "is_active" tinyint(1) not null default '1', "sort_order" integer not null default '0', "created_at" datetime, "updated_at" datetime);
...
CREATE TABLE "courses" ("id" integer primary key autoincrement not null, "title_ar" varchar not null, "title_en" varchar, "slug" varchar not null, "description_ar" text not null, "description_en" text, "objectives_ar" text, "objectives_en" text, "requirements_ar" text, "requirements_en" text, "category_id" integer not null, "instructor_id" integer not null, "price" numeric not null default '0', "discount_price" numeric, "is_free" tinyint(1) not null default '0', "start_date" datetime, "end_date" datetime, "duration_hours" integer, "max_students" integer not null default '50', "thumbnail" varchar, "banner" varchar, "intro_video" varchar, "level" varchar check ("level" in ('beginner', 'intermediate', 'advanced')) not null default 'beginner', "type" varchar check ("type" in ('online', 'offline', 'hybrid')) not null default 'online', "status" varchar check ("status" in ('draft', 'published', 'archived')) not null default 'draft', "is_featured" tinyint(1) not null default '0', "certificate_enabled" tinyint(1) not null default '1', "meta_title_ar" varchar, "meta_title_en" varchar, "meta_description_ar" text, "meta_description_en" text, "meta_keywords_ar" text, "meta_keywords_en" text, "enrolled_count" integer not null default '0', "rating" numeric not null default '0', "reviews_count" integer not null default '0', "views_count" integer not null default '0', "created_at" datetime, "updated_at" datetime, foreign key("category_id") references "categories"("id") on delete cascade, foreign key("instructor_id") references "users"("id") on delete cascade);
...
CREATE TABLE "lessons" ("id" integer primary key autoincrement not null, "course_id" integer not null, "title_ar" varchar not null, "title_en" varchar, "slug" varchar not null, "description_ar" text, "description_en" text, "type" varchar check ("type" in ('video', 'pdf', 'text', 'quiz', 'assignment', 'live_session')) not null default 'video', "video_url" varchar, "video_duration" varchar, "pdf_file" varchar, "text_content" text, "attachments" text, "live_session_date" datetime, "google_meet_link" varchar, "meeting_id" varchar, "meeting_password" varchar, "is_free" tinyint(1) not null default '0', "is_published" tinyint(1) not null default '1', "sort_order" integer not null default '0', "duration_minutes" integer not null default '0', "views_count" integer not null default '0', "completed_count" integer not null default '0', "created_at" datetime, "updated_at" datetime, foreign key("course_id") references "courses"("id") on delete cascade);
...
CREATE TABLE "enrollments" ("id" integer primary key autoincrement not null, "user_id" integer not null, "course_id" integer not null, "payment_id" integer, "status" varchar check ("status" in ('pending', 'active', 'completed', 'cancelled', 'expired')) not null default 'pending', "enrolled_at" datetime not null, "expires_at" datetime, "completed_at" datetime, "progress_percentage" integer not null default '0', "lessons_completed" integer not null default '0', "total_lessons" integer not null default '0', "quiz_attempts" integer not null default '0', "quiz_average" numeric, "live_sessions_attended" integer not null default '0', "total_live_sessions" integer not null default '0', "rating" integer, "review" text, "reviewed_at" datetime, "certificate_issued" tinyint(1) not null default '0', "certificate_issued_at" datetime, "certificate_number" varchar, "last_accessed_at" datetime, "last_lesson_id" integer, "created_at" datetime, "updated_at" datetime, "total_hours_watched" numeric not null default '0', "activated_at" datetime, foreign key("user_id") references "users"("id") on delete cascade, foreign key("course_id") references "courses"("id") on delete cascade, foreign key("payment_id") references "payments"("id") on delete set null, foreign key("last_lesson_id") references "lessons"("id") on delete set null);
...
CREATE TABLE "payments" ("id" integer primary key autoincrement not null, "user_id" integer not null, "course_id" integer not null, "payment_id" varchar not null, "invoice_number" varchar not null, "amount" numeric not null, "discount_amount" numeric not null default '0', "tax_amount" numeric not null default '0', "total_amount" numeric not null, "currency" varchar not null default 'SAR', "payment_method" varchar check ("payment_method" in ('online', 'bank_transfer', 'cash', 'free')) not null default 'online', "payment_gateway" varchar check ("payment_gateway" in ('paytabs', 'hyperpay', 'stripe', 'manual')), "status" varchar check ("status" in ('pending', 'completed', 'failed', 'cancelled', 'refunded')) not null default 'pending', "paid_at" datetime, "expires_at" datetime, "gateway_transaction_id" varchar, "gateway_reference" varchar, "gateway_response" text, "billing_data" text, "invoice_pdf" varchar, "refunded_amount" numeric not null default '0', "refunded_at" datetime, "refund_reason" varchar, "notes" text, "failure_reason" text, "created_at" datetime, "updated_at" datetime, "payment_data" text, foreign key("user_id") references "users"("id") on delete cascade, foreign key("course_id") references "courses"("id") on delete cascade);
...
CREATE TABLE "quizzes" ("id" integer primary key autoincrement not null, "course_id" integer not null, "lesson_id" integer, "title_ar" varchar not null, "title_en" varchar, "description_ar" text, "description_en" text, "duration_minutes" integer not null default '30', "passing_score" integer not null default '70', "is_active" tinyint(1) not null default '1', "allow_retake" tinyint(1) not null default '1', "max_attempts" integer not null default '3', "show_results" tinyint(1) not null default '1', "randomize_questions" tinyint(1) not null default '0', "due_date" datetime, "total_questions" integer not null default '0', "total_points" integer not null default '0', "created_at" datetime, "updated_at" datetime, foreign key("course_id") references "courses"("id") on delete cascade, foreign key("lesson_id") references "lessons"("id") on delete cascade);
...
CREATE TABLE "certificates" ("id" integer primary key autoincrement not null, "user_id" integer not null, "course_id" integer not null, "enrollment_id" integer not null, "certificate_number" varchar not null, "issued_at" datetime, "pdf_path" varchar, "verification_code" varchar not null, "is_verified" tinyint(1) not null default '0', "notes" text, "created_at" datetime, "updated_at" datetime, foreign key("user_id") references "users"("id") on delete cascade, foreign key("course_id") references "courses"("id") on delete cascade, foreign key("enrollment_id") references "enrollments"("id") on delete cascade);
...
CREATE TABLE "notifications" ("id" integer primary key autoincrement not null, "title" varchar not null, "message" text not null, "type" varchar check ("type" in ('info', 'success', 'warning', 'error')) not null default 'info', "user_id" integer, "read_at" datetime, "data" text, "created_at" datetime, "updated_at" datetime, foreign key("user_id") references "users"("id") on delete cascade);
...
CREATE TABLE "blog_posts" ("id" integer primary key autoincrement not null, "author_id" integer not null, "title_ar" varchar not null, "title_en" varchar, "slug" varchar not null, "excerpt_ar" text, "excerpt_en" text, "content_ar" text not null, "content_en" text, "featured_image" varchar, "gallery" text, "category" varchar not null default 'general', "tags" text, "status" varchar check ("status" in ('draft', 'published', 'archived')) not null default 'draft', "is_featured" tinyint(1) not null default '0', "comments_enabled" tinyint(1) not null default '1', "published_at" datetime, "meta_title_ar" varchar, "meta_title_en" varchar, "meta_description_ar" text, "meta_description_en" text, "meta_keywords_ar" text, "meta_keywords_en" text, "views_count" integer not null default '0', "likes_count" integer not null default '0', "shares_count" integer not null default '0', "reading_time" numeric not null default '0', "created_at" datetime, "updated_at" datetime, foreign key("author_id") references "users"("id") on delete cascade);
...
CREATE TABLE "settings" ("id" integer primary key autoincrement not null, "key" varchar not null, "value" text, "type" varchar not null default 'string', "group" varchar not null default 'general', "label" varchar, "description" text, "is_public" tinyint(1) not null default '0', "created_at" datetime, "updated_at" datetime);
...
CREATE TABLE "quiz_questions" ("id" integer primary key autoincrement not null, "quiz_id" integer not null, "question_ar" varchar not null, "question_en" varchar, "type" varchar check ("type" in ('multiple_choice', 'true_false', 'fill_blank', 'essay')) not null default 'multiple_choice', "options" text, "correct_answer" varchar, "explanation_ar" text, "explanation_en" text, "points" integer not null default '1', "sort_order" integer not null default '0', "is_active" tinyint(1) not null default '1', "created_at" datetime, "updated_at" datetime, foreign key("quiz_id") references "quizzes"("id") on delete cascade);
...
CREATE TABLE "quiz_attempts" ("id" integer primary key autoincrement not null, "quiz_id" integer not null, "user_id" integer not null, "attempt_number" integer not null default '1', "started_at" datetime not null, "completed_at" datetime, "score" integer not null default '0', "total_points" integer not null default '0', "percentage" numeric not null default '0', "is_passed" tinyint(1) not null default '0', "answers" text, "notes" text, "created_at" datetime, "updated_at" datetime, foreign key("quiz_id") references "quizzes"("id") on delete cascade, foreign key("user_id") references "users"("id") on delete cascade);
...
CREATE TABLE "contact_messages" ("id" integer primary key autoincrement not null, "name" varchar not null, "email" varchar not null, "phone" varchar, "subject" varchar not null, "message" text not null, "status" varchar check ("status" in ('new', 'read', 'replied', 'closed')) not null default 'new', "admin_notes" text, "read_at" datetime, "replied_at" datetime, "created_at" datetime, "updated_at" datetime);
...
CREATE TABLE "user_settings" ("id" integer primary key autoincrement not null, "user_id" integer not null, "key" varchar not null, "value" text not null, "created_at" datetime, "updated_at" datetime, foreign key("user_id") references "users"("id") on delete cascade);
...
CREATE TABLE "lesson_completions" ("id" integer primary key autoincrement not null, "enrollment_id" integer not null, "lesson_id" integer not null, "user_id" integer not null, "completed_at" datetime not null, "time_spent_minutes" integer, "progress_percentage" numeric not null default '0', "quiz_results" text, "created_at" datetime, "updated_at" datetime, foreign key("enrollment_id") references "enrollments"("id") on delete cascade, foreign key("lesson_id") references "lessons"("id") on delete cascade, foreign key("user_id") references "users"("id") on delete cascade);
...
CREATE TABLE "course_resources" ("id" integer primary key autoincrement not null, "course_id" integer not null, "lesson_id" integer, "title_ar" varchar not null, "title_en" varchar, "description_ar" text, "description_en" text, "type" varchar check ("type" in ('pdf', 'video', 'link', 'document', 'image', 'audio')) not null default 'document', "file_path" varchar, "file_name" varchar, "file_size" varchar, "external_url" varchar, "is_free" tinyint(1) not null default '1', "is_published" tinyint(1) not null default '1', "sort_order" integer not null default '0', "download_count" integer not null default '0', "created_at" datetime, "updated_at" datetime, foreign key("course_id") references "courses"("id") on delete cascade, foreign key("lesson_id") references "lessons"("id") on delete cascade);
...
CREATE TABLE "messages" ("id" integer primary key autoincrement not null, "sender_id" integer not null, "receiver_id" integer not null, "course_id" integer, "subject" varchar, "content" text not null, "type" varchar check ("type" in ('general', 'course_question', 'technical_support', 'feedback')) not null default 'general', "priority" varchar check ("priority" in ('low', 'medium', 'high', 'urgent')) not null default 'medium', "status" varchar check ("status" in ('unread', 'read', 'replied', 'closed')) not null default 'unread', "parent_id" integer, "read_at" datetime, "created_at" datetime, "updated_at" datetime, foreign key("sender_id") references "users"("id") on delete cascade, foreign key("receiver_id") references "users"("id") on delete cascade, foreign key("course_id") references "courses"("id") on delete cascade, foreign key("parent_id") references "messages"("id") on delete cascade);
