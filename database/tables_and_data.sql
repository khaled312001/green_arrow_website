...
CREATE TABLE `certificates` (`id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY not null, `user_id` int(11) not null, `course_id` int(11) not null, `enrollment_id` int(11) not null, `certificate_number` varchar(255) not null, `issued_at` timestamp, `pdf_path` varchar(255), `verification_code` varchar(255) not null, `is_verified` tinyint(1) not null default '0', `notes` longtext, `created_at` timestamp, `updated_at` timestamp, foreign key(`user_id`) references `users`(`id`) on delete cascade, foreign key(`course_id`) references `courses`(`id`) on delete cascade, foreign key(`enrollment_id`) references `enrollments`(`id`) on delete cascade);
...
CREATE TABLE `settings` (`id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY not null, `key` varchar(255) not null, `value` longtext, `type` varchar(255) not null default 'string', `group` varchar(255) not null default 'general', `label` varchar(255), `description` longtext, `is_public` tinyint(1) not null default '0', `created_at` timestamp, `updated_at` timestamp);
...
CREATE TABLE `quiz_attempts` (`id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY not null, `quiz_id` int(11) not null, `user_id` int(11) not null, `attempt_number` int(11) not null default '1', `started_at` timestamp not null, `completed_at` timestamp, `score` int(11) not null default '0', `total_points` int(11) not null default '0', `percentage` numeric not null default '0', `is_passed` tinyint(1) not null default '0', `answers` longtext, `notes` longtext, `created_at` timestamp, `updated_at` timestamp, foreign key(`quiz_id`) references `quizzes`(`id`) on delete cascade, foreign key(`user_id`) references `users`(`id`) on delete cascade);
